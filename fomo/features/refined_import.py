...
def find_latest_refined_table(
    input_dir: str,
    use_ransac: bool = False,
    alignment_override: Optional[Path] = None,
) -> Tuple[Path, Path, str]:
    """Locate the most recent refined Dynamo table and related paths."""
    ...
    if alignment_override is not None:
        latest_alignment = alignment_override
        if not latest_alignment.exists():
            raise FileNotFoundError("Specified alignment directory not found")
    else:
    ...
    if use_ransac:
        ransac_tables = sorted(
            averages_dir.glob("refined_table_ref_*_ite_*_RANSAC.tbl")
        )
        if not ransac_tables:
            raise FileNotFoundError(
                "No refined_table_ref_*_RANSAC.tbl file found"
            )
        path_to_refined_tbl = ransac_tables[0]
    else:
        refined_tables = sorted(
            p
            for p in averages_dir.glob("refined_table_ref_*_ite_*.tbl")
            if "RANSAC" not in p.name
        )
        if not refined_tables:
            raise FileNotFoundError("No refined_table_ref_*.tbl file found")
        path_to_refined_tbl = refined_tables[0]

    prefix = "refined_RANSAC" if use_ransac else "refined"

    return path_to_tomograms, path_to_refined_tbl, prefix

def _parse_refined_table(
    path_to_refined_tbl: Path, verbose: bool = False
    ...
    return tomo_numbers, xyz, shifts, eulers


def generate_refined_csvs(
    path_to_tomograms: Path,
    path_to_refined_tbl: Path,
    prefix: str,
    verbose: bool = False,
) -> None:
    """Generate per-tomogram and per-filament refined CSV files."""

    tomo_numbers, xyz, shifts, eulers = _parse_refined_table(
        path_to_refined_tbl, verbose=verbose
    )
    mod_xyz = xyz + shifts
    ...
    for tomo in np.unique(tomo_numbers):
        ...
        out_csv = volume_dir / f"{prefix}_volume_{tomo}_xyz_abg.csv"
            ...
            with open(raw_file) as rf:
                for line in rf:
                    ...
                    try:
                        ...
                        continue
                    x_key, y_key, z_key = (
                        _normalize_coord(c) for c in (x, y, z)
                    )
                    mapping.setdefault(x_key, {}).setdefault(y_key, {})[
                        z_key
                    ] = filament
        ...
        for filament, rows in per_filament.items():
            out = volume_dir / f"{prefix}_xyz_{filament}.csv"
            np.savetxt(out, np.array(rows), fmt="%.6f", delimiter=",")


def import_refined_coordinates(
    input_dir: str, verbose: bool = False, use_ransac: bool = False
) -> Tuple[Path, Path]:
    ...
    use_ransac : bool, default False
        If ``True`` use tables and output files generated by the RANSAC pipeline.
    ...
    """
    path_to_tomograms, path_to_refined_tbl, prefix = find_latest_refined_table(
        input_dir, use_ransac=use_ransac
    )
    generate_refined_csvs(
        path_to_tomograms, path_to_refined_tbl, prefix, verbose=verbose
    )
